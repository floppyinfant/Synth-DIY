...
prog_uint16_t antilogTable[] PROGMEM = {
...
prog_uint16_t midiTable[] PROGMEM = {
      ...
      for (byte i = 0; i < 4; i++)
      {
        if (justreleased[i] && longPress != 0)
        {
          if (bank < 8 && checkLocation((bank * 4) + i)) // only cue a patch if the location is in use
          {
            ...
            playingBank = bank;
            if (triggerChannel != 0)
              MIDI.sendNoteOn(64 + (bank * 4) + i, 127, triggerChannel); // send a note with velocity 127
          }
          else if (bank >= 8)          
          {
    ...
    if (seqNotes[seqCurrentStep] != 127) // do this code only if this step is not tied (to the previous)
    {
      ...
      else // if we got this far, we can safely change the pitch send out a midi note 
      {
        if (playEdited && lastEdited != 255) // should we play the note being edited
        {
          ...
          { 
            if (checkAccent(lastEdited)) // does the step have an accent?
              MIDI.sendNoteOn(midiOn, 127, noteChannel); // send a note with velocity 127
            else
              MIDI.sendNoteOn(midiOn, map(accentLevel, 0, 255, 127, 0), noteChannel); // send a note with velocity 100
          }
        }
        ...
        else
        {
          ...
          {
            MIDI.sendNoteOff(midiOn, 0, noteChannel); // turn off the note we last turned on
          }
